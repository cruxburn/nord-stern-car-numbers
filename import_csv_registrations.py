...
def import_csv_registrations(csv_file_path):
    ...
    try:
        with open(csv_file_path, 'r', encoding='utf-8') as csvfile:
            ...
            for row_num, row in enumerate(reader, start=2):  # Start at 2 since row 1 is header
                ...
                try:
                    ...
                    car_make = row.get(' Make', '').strip()  # Note: there's a leading space in the column name
                    ...
                    if not first_name or not last_name:
                        print(f"⚠️  Row {row_num}: Skipping - missing first or last name")
                    ...
                    try:
                        ...
                        int(car_number)  # This validates it's a number but doesn't change the format
                        car_number = car_number.strip()  # Just remove any whitespace
                    except ValueError:
                        print(f"⚠️  Row {row_num}: Skipping - invalid car number: {car_number}")
                        stats['errors'] += 1
                        continue
                    ...
                    cursor.execute('SELECT id FROM car_registrations WHERE car_number = ?', (car_number,))
                    existing = cursor.fetchone()
                    if existing:
                        print(f"⚠️  Row {row_num}: Duplicate car number {car_number} - importing anyway")
                    ...
                    if reserved_date:
                        if reserved_date.lower() == 'retired':
                            ...
                            try:
                                ...
                                date_formats = ['%m/%d/%y', '%m/%d/%Y', '%Y-%m-%d', '%m-%d-%Y', '%Y/%m/%d']
                                parsed_date = None
                                
                                for fmt in date_formats:
                                    try:
                                        parsed_date = datetime.strptime(reserved_date, fmt)
                                        break
                                    except ValueError:
                                        continue
                                ...
                                if parsed_date:
                                    ...
                                    print(f"⚠️  Row {row_num}: Invalid reserved date format: {reserved_date}")
                                    reserved_date = None
                            except Exception as e:
                                print(f"⚠️  Row {row_num}: Error parsing reserved date: {e}")
                                reserved_date = None
                    else:
                        # If no reserved date, use default date 03/28/1959
                        reserved_date = "1959-03-28"
                        print(f"📅 Row {row_num}: No reserved date found, using default: 03/28/1959")
                    ...
                    if last_used and last_used.lower() in ['x', 'yes', 'true', '1']:
                        ...
                        print(f"✅ Row {row_num}: {first_name} {last_name} (#{car_number}) - 2025 usage recorded")
                    else:
                        print(f"✅ Row {row_num}: {first_name} {last_name} (#{car_number}) - no 2025 usage")
                    ...
                    cursor.execute('''
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (first_name, last_name, car_number, sort_order, car_make, car_model, car_year_int,
                          car_color, reserved_date, notes, status, last_usage_year, expiration_date,
                          usage_count, is_active_in_period))
        ...
        for reg in sample_data:
            first_name, last_name, car_number, reserved_date, expiration_date, last_usage_year, usage_count, is_active = reg
