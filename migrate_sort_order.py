...
def migrate_sort_order():
    ...
    try:
        ...
        if "sort_order" not in columns:
            ...
            cursor.execute(
                "ALTER TABLE car_registrations ADD COLUMN sort_order INTEGER"
            )
        ...
        for reg_id, car_number in registrations:
            try:
                ...
                updated_count += 1
                print(
                    f"  ✅ Updated registration {reg_id}: {car_number} -> sort_order {sort_order}"
                )

            except ValueError as e:
                print(
                    f"  ⚠️  Warning: Could not convert car_number '{car_number}' to integer for registration {reg_id}"
                )
