...
from app import app, init_db, calculate_expiration_date, update_usage_for_registration, remove_usage_for_registration
...
class NordSternCarNumbersTestCase(unittest.TestCase):
    ...
    def _insert_test_data(self):
        ...
        test_data = [
            ...
            ('John', 'Doe', '101', 101, 'BMW', 'M3', 2020, 'Black', '2022-01-15', 2025, 'Active', 'Test registration 1', 2025, '2028-01-01', 1, True),
            ('Jane', 'Smith', '102', 102, 'Porsche', '911', 2021, 'Red', '2023-01-16', 2025, 'Active', 'Test registration 2', None, '2026-01-01', 0, True),
            ('Bob', 'Johnson', '103', 103, 'Audi', 'RS4', 2019, 'Silver', '2020-01-10', 2025, 'Retired', 'Test registration 3', 2024, '2027-01-01', 1, True),
            ('Alice', 'Brown', '104', 104, 'BMW', 'M2', 2022, 'Blue', '2021-01-17', 2025, 'Active', 'Test registration 4', None, '2024-01-01', 0, False),
            ('Charlie', 'Wilson', '105', 105, 'Porsche', 'Cayman', 2020, 'White', '2024-01-18', 2025, 'Active', 'Test registration 5', 2025, '2028-01-01', 2, True),
            ('Sarah', 'Johnson', '106', 106, 'Audi', 'TT RS', 2021, 'Gray', '2022-01-19', 2025, 'Active', 'Test registration 6', None, '2025-01-01', 0, True),
            ('Michael', 'Chen', '107', 107, 'BMW', 'M4', 2023, 'Green', '2023-01-20', 2025, 'Active', 'Test registration 7', 2025, '2028-01-01', 1, True),
            ('Lisa', 'Martinez', '108', 108, 'Porsche', 'Boxster', 2018, 'Yellow', '2020-01-21', 2025, 'Active', 'Test registration 8', None, '2023-01-01', 0, False),
            ('David', 'Thompson', '109', 109, 'Audi', 'S4', 2020, 'White', '2021-01-22', 2025, 'Active', 'Test registration 9', 2024, '2027-01-01', 1, True),
            ('Emma', 'Davis', '110', 110, 'BMW', 'M5', 2021, 'Blue', '2022-01-23', 2025, 'Active', 'Test registration 10', None, '2025-01-01', 0, True),
            ('Leading', 'Zero', '022', 22, 'Porsche', 'GT4', 2022, 'Red', '2022-12-21', 2025, 'Active', 'Test with leading zero', 2025, '2028-01-01', 1, True),
        ]
    ...
    def test_add_registration_success(self):
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT last_usage_year, usage_count, expiration_date, is_active_in_period FROM car_registrations WHERE car_number = ?', ('999',))
    ...
    def test_add_registration_missing_required_fields(self):
        ...
        response = self.client.post('/add', data=data)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'First name, last name, and car number are required', response.data)
    ...
    def test_car_number_leading_zeros(self):
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT car_number FROM car_registrations WHERE first_name = ? AND last_name = ?', ('Test', 'User'))
        result = cursor.fetchone()
        conn.close()
    ...
    def test_record_usage_api(self):
        ...
        response = self.client.post('/api/record_usage/2', data={'usage_year': '2025'}, follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT last_usage_year, usage_count FROM car_registrations WHERE id = ?', (2,))
        result = cursor.fetchone()
        conn.close()
    ...
    def test_record_usage_api_default_year(self):
        ...
        response = self.client.post('/api/record_usage/2', data={}, follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        self.assertIn(f'Usage recorded successfully for {current_year}'.encode(), response.data)
    ...
    def test_record_usage_nonexistent_registration(self):
        ...
        response = self.client.post('/api/record_usage/999', data={'usage_year': '2025'}, follow_redirects=True)
        self.assertEqual(response.status_code, 200)
