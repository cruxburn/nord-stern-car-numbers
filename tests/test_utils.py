...
class UtilsTestCase(unittest.TestCase):
    ...
    def test_calculate_expiration_date_different_formats(self):
        ...
        test_cases = [
            ('2022-01-15', 2025, '2028-01-01'),  # 2025 usage extends 2025 expiration to 2028
            ('2023-06-30', 2024, '2029-01-01'),  # 2024 usage extends 2026 expiration to 2029
            ('2021-12-31', 2026, '2030-01-01'),  # 2026 usage extends 2024 expiration to 2030
        ]
    ...
    def test_update_usage_for_registration_multiple_updates(self):
        ...
        self.assertEqual(result[2], '2031-01-01')  # expiration_date (based on latest usage)
    ...
    def test_update_usage_for_registration_default_year(self):
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT last_usage_year FROM car_registrations WHERE id = ?', (1,))
        result = cursor.fetchone()
        conn.close()
    ...
    def test_remove_usage_for_registration_no_usage(self):
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT last_usage_year, usage_count FROM car_registrations WHERE id = ?', (1,))
        result = cursor.fetchone()
        conn.close()
    ...
    def test_expiration_date_recalculation(self):
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT expiration_date FROM car_registrations WHERE id = ?', (1,))
        result = cursor.fetchone()
        conn.close()
        
        self.assertIsNone(result[0])  # Not automatically calculated during direct DB insertion
    ...
    def test_is_active_in_period_calculation(self):
        ...
        cursor.execute('''
            ...
            (first_name, last_name, car_number, sort_order, car_make, car_model, car_year, car_color, reserved_date, notes)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', ('John', 'Doe', '101', 101, 'BMW', 'M3', 2020, 'Black', f'{current_year}-01-15', 'Test'))
        conn.commit()
        conn.close()
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT is_active_in_period FROM car_registrations WHERE id = ?', (1,))
        result = cursor.fetchone()
        conn.close()
        ...
        cursor.execute('''
            ...
            (first_name, last_name, car_number, sort_order, car_make, car_model, car_year, car_color, reserved_date, notes)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', ('Jane', 'Smith', '102', 102, 'Porsche', '911', 2020, 'Red', f'{old_year}-01-15', 'Test'))
        conn.commit()
        conn.close()
        
        # Update usage to trigger expiration calculation - use a year that will make it expired
        update_usage_for_registration(2, old_year + 2)  # Usage 2 years after reservation
        
        # Should be expired (3 years from usage = old_year + 5, which is less than current_year)
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT is_active_in_period FROM car_registrations WHERE id = ?', (2,))
        result = cursor.fetchone()
        conn.close()
