...
def test_rolling_period():
    ...
    expiration_date_with_usage, is_active_with_usage = calculate_expiration_date(reserved_date, usage_date)
    print(f"   Reserved Date: {reserved_date}")
    ...
    period_from_reserved = (datetime.strptime(expiration_date, '%Y-%m-%d') - reserved_dt).days
    period_from_usage = (expiration_dt - usage_dt).days
    
    print(f"   Period from reservation: {period_from_reserved} days (~{period_from_reserved/365:.1f} years)")
    print(f"   Period from usage: {period_from_usage} days (~{period_from_usage/365:.1f} years)")
    ...
    try:
        expiration_invalid, is_active_invalid = calculate_expiration_date("invalid-date")
