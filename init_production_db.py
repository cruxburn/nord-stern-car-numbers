...
def init_production_database(db_path, data_file=None):
    ...
    try:
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_car_number ON car_registrations(car_number)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_sort_order ON car_registrations(sort_order)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_name ON car_registrations(first_name, last_name)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_status ON car_registrations(status)')
    ...
    try:
        ...
        for reg in registrations:
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                reg['id'], reg['first_name'], reg['last_name'], reg['car_number'], 
                reg['sort_order'], reg['car_make'], reg['car_model'], reg['car_year'], 
                reg['car_color'], reg['reserved_date'], reg['reserved_for_year'], 
                reg['status'], reg['notes'], reg['last_usage_year'], reg['expiration_date'], 
                reg['usage_count'], reg['is_active_in_period'], reg['created_at'], 
                reg['updated_at']
            ))
