    ...
    try:
        ...
        for column in columns_to_add:
            if column == "last_usage_date":
                cursor.execute(
                    "ALTER TABLE car_registrations ADD COLUMN last_usage_date DATE"
                )
            elif column == "expiration_date":
                cursor.execute(
                    "ALTER TABLE car_registrations ADD COLUMN expiration_date DATE"
                )
            elif column == "usage_count":
                cursor.execute(
                    "ALTER TABLE car_registrations ADD COLUMN usage_count INTEGER DEFAULT 0"
                )
            elif column == "is_active_in_period":
                cursor.execute(
                    "ALTER TABLE car_registrations ADD COLUMN is_active_in_period BOOLEAN DEFAULT 1"
                )
        ...
        for reg_id, reserved_date, status in registrations:
            if reserved_date:
                ...
                try:
                    ...
                    print(
                        f"⚠️  Warning: Could not parse reserved_date '{reserved_date}' for record {reg_id}: {e}"
                    )
                    continue
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM car_registrations WHERE expiration_date IS NOT NULL"
        )
