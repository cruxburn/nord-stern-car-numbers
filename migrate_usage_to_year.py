...
def migrate_usage_to_year():
    ...
    try:
        ...
        cursor.execute('SELECT id, reserved_date, last_usage_year FROM car_registrations')
        ...
        # Show summary
        cursor.execute('SELECT COUNT(*) FROM car_registrations WHERE last_usage_year IS NOT NULL')
