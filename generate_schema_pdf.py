...
def create_schema_pdf():
    ...
    story.append(
        Paragraph("Drivers Education Car Number Management System", normal_style)
    )
    ...
    story.append(
        Paragraph("• Single table design for maximum simplicity", normal_style)
    )
    story.append(
        Paragraph("• SQLite database for portability and reliability", normal_style)
    )
    ...
    field_descriptions = [
        [
            "Primary Key",
            "id",
            "Auto-incrementing unique identifier for each registration",
        ],
        ...
        [
            "Optional Fields",
            "car_make",
            "Vehicle manufacturer (e.g., BMW, Porsche, Audi)",
        ],
    ...
    field_table = Table(
        field_descriptions, colWidths=[1.2 * inch, 1.5 * inch, 3.3 * inch]
    )
    ...
    constraints = [
        ...
        [
            "Unique",
            "car_number",
            "No duplicate car numbers allowed",
            "Prevents conflicts in car number assignments",
        ],
        [
            "Not Null",
            "first_name",
            "Must have a value",
            "Ensures driver identification",
        ],
        ...
        [
            ...
            "status",
            "Defaults to 'Active'",
            "Sets initial status automatically",
        ],
        [
            "Auto",
            "created_at",
    ...
    constraint_table = Table(
        constraints, colWidths=[1.2 * inch, 1.2 * inch, 2 * inch, 1.6 * inch]
    )
    constraint_table.setStyle(
        TableStyle(
            [
    ...
    story.append(
        Paragraph("• Supports both positive and negative numbers", normal_style)
    )

    story.append(Spacer(1, 12))
    story.append(Paragraph("Status Values:", subheading_style))
    story.append(
        Paragraph("• Active: Currently registered and participating", normal_style)
    )
    story.append(
        Paragraph("• Retired: No longer active but record preserved", normal_style)
    )
    ...
    story.append(
        Paragraph("• created_at: Set once when record is created", normal_style)
    )
    story.append(
        Paragraph("• updated_at: Updated every time record is modified", normal_style)
    )
    story.append(
        Paragraph("• Both use SQLite's CURRENT_TIMESTAMP function", normal_style)
    )
