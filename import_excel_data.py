...
def import_excel_data():
    ...
    # Read the Excel file
    try:
        df = pd.read_excel('/Users/andybarker/Downloads/2025 Car Number Registration 2025-02-04.xlsx')
    ...
    for index, row in df.iterrows():
        ...
        for col_idx in range(len(row) - 10, len(row)):
            if pd.notna(row.iloc[col_idx]):
                ...
                if cell_value and cell_value != 'nan':
                    ...
                    if not car_make and any(brand in cell_value.lower() for brand in ['bmw', 'porsche', 'audi', 'mercedes', 'ferrari', 'lamborghini', 'toyota', 'honda', 'ford', 'chevrolet']):
                        car_make = cell_value
                    elif not car_model and len(cell_value) <= 10:
                        car_model = cell_value
                    elif not car_year and cell_value.isdigit() and 1900 <= int(cell_value) <= 2030:
        ...
        try:
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (first_name, last_name, car_number, car_make, car_model, car_year, car_color, status, datetime.now().date()))
