    ...
    try:
        ...
        if last_usage_year:
            ...
            try:
                ...
                print(f"⚠️  Error calculating expiration date: invalid usage year '{last_usage_year}'")
                return None, False
            ...
            if last_usage_year > original_expiration_year:
                ...
                periods_passed = (years_since_original + 5) // 3  # Round up to get 2 periods for 2024
...
def update_usage_for_registration(registration_id, usage_year=None):
    ...
    try:
        ...
        expiration_date, is_active = calculate_expiration_date(reserved_date, usage_year)
        ...
        conn.commit()
        print(f"✅ Updated usage for registration {registration_id} (year: {usage_year})")
        return True
...
def search():
    ...
    if query and not show_all:
        ...
        cursor.execute('SELECT * FROM car_registrations ORDER BY sort_order, car_number')
    
    registrations = cursor.fetchall()
    conn.close()
    
    return render_template('search.html', registrations=registrations, query=query, number=number, show_all=show_all)
...
def add_registration():
    if request.method == 'POST':
        ...
        try:
            ...
            car_number = str(int(car_number)).zfill(3)  # Ensure 3 digits with leading zeros
            if car_year:
        ...
        cursor.execute('SELECT * FROM car_registrations WHERE car_number = ?', (car_number,))
        if cursor.fetchone():
        ...
        if reserved_date:
            expiration_date, is_active_in_period = calculate_expiration_date(reserved_date)
        ...
        flash(f'Registration added successfully for {first_name} {last_name} with car number {car_number}!', 'success')
...
def edit_registration(id):
    ...
    if request.method == 'POST':
        ...
        cursor.execute('SELECT * FROM car_registrations WHERE car_number = ? AND id != ?', (car_number, id))
        if cursor.fetchone():
            flash(f'Car number {car_number} is already taken!', 'error')
            conn.close()
            return render_template('edit.html', registration=None)
        ...
        if reserved_date:
            ...
            cursor.execute('SELECT last_usage_year FROM car_registrations WHERE id = ?', (id,))
            current_usage = cursor.fetchone()
            last_usage_year = current_usage[0] if current_usage else None
            
            expiration_date, is_active_in_period = calculate_expiration_date(reserved_date, last_usage_year)
        ...
        cursor.execute('''
            ...
            WHERE id=?
        ''', (first_name, last_name, car_number, sort_order, car_make, car_model, car_year, car_color, reserved_date, status, notes, expiration_date, is_active_in_period, id))
...
def delete_registration(id):
    ...
    cursor.execute('SELECT first_name, last_name, car_number FROM car_registrations WHERE id = ?', (id,))
    registration = cursor.fetchone()
    
    if registration:
        cursor.execute('DELETE FROM car_registrations WHERE id = ?', (id,))
        conn.commit()
        flash(f'Registration for {registration[0]} {registration[1]} (Car #{registration[2]}) deleted successfully!', 'success')
    else:
        flash('Registration not found!', 'error')
...
def check_number(number):
    ...
    cursor.execute('SELECT * FROM car_registrations WHERE car_number = ?', (search_number,))
    registration = cursor.fetchone()
    conn.close()
